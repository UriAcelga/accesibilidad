...
<body class="h-screen bg-verde-azulado print:text-sm print:bg-white print:text-black">
    <x-app-navbar :titulos="['Estudiantes', 'Solicitudes']" :titulosHrefUrl="[route('home'), route('solicitud')]" selected="Estudiantes"></x-app-navbar>
    {{-- Filtros de busqueda --}}
    <div
        class="w-5/6 lg:w-4/5 h-4/5 md:h-2/3 mx-auto flex mt-8 md:mt-12  print:m-0 font-roboto border-2 rounded-lg border-azul-oscuro">
        <div class="h-full w-full flex flex-col">
            <span class="text-white w-full bg-azul-oscuro text-center text-lg">Filtros de búsqueda</span>
            <form method="GET" class="w-full h-full p-4 bg-azul-marino-alterno flex flex-col md:flex-row"
                x-data="{}"
                @submit.prevent="
            ...
            ">
                <div class="w-full md:w-2/3 h-full flex flex-col">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="filter[buscar]" value="{{ request('filter.buscar') }}"
                                ...
                                class="block w-full h-12 pl-10 pr-3 py-2 border-2 border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        </div>
                    </div>
                    <div class="w-full">
                    ...
                    <div class="w-full">
                        <x-dropdown-interno :facultades="$facultades" prompt="Filtrar por Carrera"
                            field_name="filter[carrera_id]" label="Carrera"></x-dropdown-interno>
                    </div>
                    <div class="flex flex-row space-x-4">

                        <div class="w-1/2">
                            <label for="filter[fecha_desde]"
                                class="block mb-2 text-lg font-medium text-white">Desde:</label>
                            <input type="date" name="filter[fecha_desde]" min="2025-01-01"
                                max="{{ now()->format('Y-m-d') }}" class="block w-full h-12 pl-10 pr-3 py-2 border-2 border-gray-300 rounded-md bg-white">
                        </div>
                        <div class="w-1/2">
                            <label for="filter[fecha_hasta]"
                                class="block mb-2 text-lg font-medium text-white">Hasta:</label>
                            <input type="date" name="filter[fecha_hasta]" min="2025-01-01"
                                max="{{ now()->format('Y-m-d') }}" class="block w-full h-12 pl-10 pr-3 py-2 border-2 border-gray-300 rounded-md bg-white">
                        </div>
                    </div>
                </div>
                <div class="mt-8 md:mt-auto w-full md:w-1/3 h-full flex flex-row md:flex-col text-center items-center justify-center space-x-4 md:space-x-0 space-y-0 md:space-y-8 md:border-l-2 md:border-azul-oscuro">
                    <button type="submit"
                        class="px-4 py-2 h-12 w-3/5 bg-coral text-white rounded-md hover:bg-coral-oscuro focus:outline-none focus:ring-2 focus:ring-blue-500">
                        Filtrar
                    </button>
                    <a href="{{ route('home') }}"
                        class="inline-flex items-center justify-center px-4 py-2 h-12 bg-gray-300 w-3/5 text-gray-700 rounded-md hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        Limpiar
                    </a>
                </div>
            </form>
    ...
    <div role="region" aria-labelledby="Cap1" tabindex="0"
        class="w-5/6 lg:w-4/5 mx-auto flex flex-col my-8 md:my-12 print:m-0 font-roboto overflow-x-auto outline-2 outline-azul-oscuro focus:outline-dashed focus:outline-white">
        <table id="registros border-collapse">
            <caption id="Cap1"
                class="text-lg pl-2 md:pl-0 md:ml-4 text-left font-medium text-white print:text-black bg-azul-oscuro print:bg-transparent md:bg-transparent font-bold md:font-normal">
